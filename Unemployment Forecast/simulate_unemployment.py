...
# Read the data
df = pd.read_csv('/Users/jaydenmcnab/Projects/oreo/data/Connecticut Unemployed.csv')
...
def fit_and_forecast(data, town_name, periods=12):
    try:
...
for town in tqdm(towns):
    ...
    forecast, conf_int = fit_and_forecast(town_data['Unemployed'], town)
...
# Save to CSV
output_file = 'unemployment_with_projections.csv'
...
yearly_avg = combined_df.groupby([combined_df['Date'].dt.year, 'Projected'])['Unemployed'].mean().round(1)
print(yearly_avg)
